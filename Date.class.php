...
class Date extends DateClass
{
	...
	function calculateTime($dateStart,$dateStop)
	{
		$Second =floor($this->Seconds($dateStart,$dateStop));
		$Minutes =floor($this->Minutes($dateStart,$dateStop));
		$Hours =floor($this->Hours($dateStart,$dateStop));
		$Days =floor($this->Days($dateStart,$dateStop));
		$Month =floor($this->Months($dateStart,$dateStop));
		$Years =floor($this->Years($dateStart,$dateStop));
		...
		if ($Second>59)
		{
			if ($Minutes>59)
			{
				if ($Hours>24)
				{
					...
					{
						if ($Month>12)
							return $Years." tahun ".$Month." bulan lalu";
						else
					...
					return $Hours." jam ".($Minutes-($Hours*60))." menit lalu";
			}
			else
				return $Minutes." menit ".($Second-($Minutes*60))." detik lalu";
		}
		else
